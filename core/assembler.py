...
class Assembler:
    ...
    def get_label_addr(self, label_name):
        addr = self.symbol_table.get(label_name)
        if addr is None:
    ...
    def encode(self, filepath):
        ...
        for line in lines:
            ...
            while args and args[0].endswith(':'):
                self.symbol_table[args[0][:-1]] = curr_line
        ...
        for line in lines:
            try:
                ...
                if cmd == 'ecall':
                    ...
                    need_args(args, 2)
                    rd = get_reg_id(args[1])
                    imm = self.get_label_addr(args[2]) - curr_line
                    ...
                    imm = self.get_label_addr(args[3]) - curr_line
                    # Jump unit: line. No implicit tailing one
                    rd = imm & 0x1F
                    func7 = (imm >> 5) & 0x7F
                elif inst_type == 'U':
